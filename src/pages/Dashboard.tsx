...
function fromBase64Unicode(str: string) {
...
export const Dashboard: React.FC = () => {
  ...
  const [facts, setFacts] = useState((location.state && (location.state as any).facts) || {});
  const [kpis, setKpis] = useState((location.state && (location.state as any).kpis) || {});
  ...
  useEffect(() => {
    ...
    if (encodedData) {
      try {
        const data = JSON.parse(fromBase64Unicode(encodedData));
        setFacts(data.facts || {});
        setKpis(data.kpis || {});
        setBarData(data.barData || []);
      } catch (error) {
        console.error('Error parsing URL data:', error);
        // Fall back to navigation state if URL data is invalid
      }
    }
  }, []);
  ...
  const handleShare = async () => {
    try {
      ...
      const encodedData = toBase64Unicode(JSON.stringify(shareData));
      ...
      const shareData = { facts, kpis, barData };
      const encodedData = toBase64Unicode(JSON.stringify(shareData));
      const shareUrl = `${window.location.origin}${window.location.pathname}?data=${encodedData}`;
  ...
  return (
    <div className="dashboard-root">
      ...
      <img src="public/rose2.png" alt="corner" className="dashboard-corner topright flip-horizontal" />
      <img src="public/rose2.png" alt="corner" className="dashboard-corner bottomleft" />
      <img src="public/rose2.png" alt="corner" className="dashboard-corner bottomright flip-horizontal" />
      ...
      <div className="dashboard-center-rect">
        ...
        <div className="dashboard-summary">
          We'll let you derive your own conclusions, or{' '}
          <span 
            className="dashboard-share" 
            onClick={handleShare}
            style={{ cursor: 'pointer' }}
          >
            {showCopied ? 'Copied! 🌹' : 'share these results 🌹'}
          </span>
        </div>
      </div>
    </div>
